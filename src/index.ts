...
async function handleRequest(request: Request, env: Env): Promise<Response> {
  ...
  console.log(
    `[${requestId}] D1 Worker received request: ${request.method} ${path}`
  );
  ...
  if (internalKey) {
    ...
    if (!providedKey || providedKey !== internalKey) {
      console.warn(
        `[${requestId}] Unauthorized request to ${path} - invalid or missing internal auth key`
      );
  ...
  try {
    ...
    switch (path) {
      case "/query": {
        ...
        try {
          ...
          console.error(
            `[${requestId}] Payload parsing error for /query:`,
            errorMsg
          );
        ...
        if (query.startsWith("SELECT")) {
          ...
          console.log(
            `[${requestId}] D1 SELECT result: success=${result.success}`
          );
          if (!result.success) {
          ...
          console.log(
            `[${requestId}] D1 write result: success=${result.success}, changes=${result.meta?.changes}`
          );
          if (!result.success) {
          ...
          console.warn(
            `[${requestId}] Unsupported query type starting with: ${query.substring(0, 10)}...`
          );
          return createJsonResponse(
            {
              success: false,
      ...
      case "/api/dashboard/stats": {
        ...
        try {
          ...
          const recentActivityData = await env.DB.prepare(
      ...
      case "/batch": {
        ...
        try {
          ...
          if (!payload || !Array.isArray(payload.statements)) {
            throw new Error(
              "Invalid payload: missing or invalid 'statements' array."
            );
          }
          // Basic check on statements array contents
          if (
            payload.statements.some((s) => !s || typeof s.query !== "string")
          ) {
            throw new Error(
              "Invalid payload: one or more statements missing 'query'."
            );
          }
        } catch (e: unknown) {
          const errorMsg =
            e instanceof Error ? e.message : "Invalid JSON payload";
          console.error(
            `[${requestId}] Payload parsing error for /batch:`,
            errorMsg
          );
          return createJsonResponse({ success: false, error: errorMsg }, 400);
        }
        ...
        console.log(
          `[${requestId}] Executing D1 batch with ${payload.statements.length} statements.`
        );
        ...
        console.log(
          `[${requestId}] D1 batch result: overallSuccess=${overallSuccess}`
        );
        ...
        if (!overallSuccess) {
          ...
          const firstError =
            ...
            "One or more batch statements failed";
      ...
      case "/health": {
        ...
        try {
          ...
          return createJsonResponse({
            success: true,
            result: { status: "ok", service: "d1-worker" },
          });
        } catch (e: any) {
          return createJsonResponse(
    ...
    const errorMsg =
      error instanceof Error
        ? error.message
        : String(error || "Unknown D1 worker error");
